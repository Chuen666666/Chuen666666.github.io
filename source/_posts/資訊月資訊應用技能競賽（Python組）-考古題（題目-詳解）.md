---
title: 資訊月資訊應用技能競賽（Python 組）- 考古題（題目 + 詳解）
categories:
  - [筆記, 程式, 題解]
tags:
  - 程式
...
$$4\displaystyle\sum_{k=1}^{n}\frac{(-1)^{k+1}}{2k-1}$$
...
#### 範例輸出1
（下圖中的 `.` 為 `空格`）
![image](https://hackmd.io/_uploads/B19kzT5cee.png)
#### 範例輸入2
```
4
41
...
#### 範例輸出2
（下圖中的 `.` 為 `空格`）
![image](https://hackmd.io/_uploads/Bk0zMT9qex.png)
#### 題解
{% fold info @ 解答 %}
...
迴文質數：既是質數又是迴文數的整數
質數：在大於 1 的自然數中，除了 1 和該數本身外，無法被其他自然數
整除的數
...
#### 範例輸出1
（下圖中的 `.` 為 `空格`）
![image](https://hackmd.io/_uploads/ryR84p5cxx.png)
#### 範例輸入2
```
200
```
#### 範例輸出2
（下圖中的 `.` 為 `空格`）
![image](https://hackmd.io/_uploads/HyG9ETqcex.png)
#### 題解
{% fold info @ 解答 %}
這題我認為難點不在邏輯上，而是輸出格式
但總體而言，它的難度還不如上一題
...
$$
\displaystyle\sum_{k=2}^{n}\frac{1}{\sqrt{k-1}+\sqrt{k}}
$$
那麼程式碼也就很好想了
```python=
n = int(input())
...
![image](https://hackmd.io/_uploads/BJgbpz-Rxe.png)
#### 輸入說明
