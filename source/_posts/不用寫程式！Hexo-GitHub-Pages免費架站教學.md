...
它的專長是用來寫數學、物理、化學的各種符號或公式，例如 $x=\frac{-b\pm\sqrt{b^2-4ac}}{2a}$。
...
Hexo 框架也有許多現存的模板可以直接套，對於不會寫 CSS ~~或沒什麼美感~~的人而言，它是一個快速建立好看網站的工具。
...
> 特別注意，因為標題已經交給 front-matter 了，因此文章從 H2（`##`）標題寫起即可
