...
- 等差中項：前後項平均數
- 等差級數：$S\_n=\frac{(a\_1+a\_n)\,n}{2}=\frac{(2\,a\_1+(n-1)\,d)\,n}{2}$（梯形公式）

### 等比
- 等比數列（G.P.）：$a\_n=a\_m\times r^{n-m}$
- 等比中項：$b=\pm\sqrt{ac}$
- 等比級數：{% raw %}$S_n=\begin{cases}n\times a_1&\text{, if }r=1\\ \frac{a_1\,(1-r^n)}{1-r}&\text{, if }r\ne1\end{cases}${% endraw %}
...
### 二項式定理
- $(x+y)^n=C\_0^n\,x^n\,y^0+C\_1^n\,x^{n-1}\,y^1+C\_2^n\,x^{n-2}\,y^2+\cdots+C\_n^n\,x^0\,y^n$

### 組合級數
令 $f(x)=(1+x)^n=C\_0^n+C\_1^n\,x^1+C\_2^n\,x^2+\cdots+C\_n^n\,x^n$
...
### 期望值
- $E=x\_1\,P\_1+x\_2\,P\_2+\cdots x\_n\,P\_n$（想像**單個樣本點占的比例**）
